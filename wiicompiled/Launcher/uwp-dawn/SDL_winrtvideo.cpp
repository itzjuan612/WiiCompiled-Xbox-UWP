...
bool WINRT_CreateWindow(SDL_VideoDevice *_this, SDL_Window *window, SDL_PropertiesID create_props)
{
    ...
    data->swapChainBackgroundPanel = NULL;
    ...
    */
    ...
    if (WINRT_XAMLWasEnabled) {
        data->swapChainBackgroundPanel = (IUnknown *)WINRT_GetSwapChainBackgroundPanelNative();
        if (data->swapChainBackgroundPanel) {
            SDL_SetPointerProperty(SDL_GetWindowProperties(window),
                                   SDL_PROP_WINDOW_WINRT_SWAPCHAIN_BACKGROUND_PANEL_POINTER,
                                   data->swapChainBackgroundPanel);
            WINRT_DetachSwapChainBackgroundPanelNative();
        }
    }
...
void WINRT_DestroyWindow(SDL_VideoDevice *_this, SDL_Window *window)
{
    ...
    if (data) {
        if (data->swapChainBackgroundPanel) {
            data->swapChainBackgroundPanel->Release();
            data->swapChainBackgroundPanel = NULL;
        }
