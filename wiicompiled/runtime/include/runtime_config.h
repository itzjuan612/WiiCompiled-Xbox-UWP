...
// Optional packager-supplied seed for the first-run Config.toml. When this file sits next to the
// executable (inside the appx for packaged builds), its contents are copied verbatim into the user
// configuration on first launch instead of the built-in template below, so an installation can ship
// sensible defaults (paths, resolution, button bindings). It is only consulted when the user config
// does not exist yet; once created the user config is authoritative and is never overwritten.
inline constexpr const char* kBundledDefaultConfigFileName = "Config.default.toml";
...
#elif defined(_WIN32)
    // Packaged (Store/UWP/Xbox) apps: delegate to the platform layer, which
    // resolves the per-package LocalAppData location via the redirected
    // LOCALAPPDATA environment variable (the OS rewrites the AppData env
    // vars for packaged processes).
...
inline void EnsureConfigFile() {
    ...
    // Prefer a packager-supplied default shipped next to the executable (inside the appx for
    // packaged builds). Fall back to the built-in template when it is absent, empty, or unreadable.
    if (const auto executableDirectory = ExecutableDirectory()) {
        std::ifstream bundled(*executableDirectory / kBundledDefaultConfigFileName, std::ios::binary);
        if (bundled) {
            const std::string seeded((std::istreambuf_iterator<char>(bundled)),
