...
{
#if defined(_MSC_VER)
    // MSVC /O1//O2 mis-folds the _mm_shuffle_ps(...,_MM_SHUFFLE(1,1,1,1)) formulation of this
    // broadcast to literal zeros (verified in isolation), which zeroes every scalar lane read
    // out of a paired register. Extract the HIGH 32 bits (ps0) with memcpy bit-surgery instead.
    uint64_t bits = 0;
    std::memcpy(&bits, &value, sizeof(bits));
    return _mm_set1_ps(PpcBitCastToFloatInline(static_cast<uint32_t>(bits >> 32)));
...
{
#if defined(_MSC_VER)
    // See PpcBroadcastPs0Inline: extract the LOW 32 bits (ps1) with memcpy bit-surgery.
    uint64_t bits = 0;
    std::memcpy(&bits, &value, sizeof(bits));
    return _mm_set1_ps(PpcBitCastToFloatInline(static_cast<uint32_t>(bits)));
#elif defined(__x86_64__) || defined(_M_X64)
    const __m128 lanes = PpcPsToM128Inline(value);
...
#if defined(_MSC_VER)
    // MSVC /O1//O2 mis-folds the lane broadcast here to zero (see PpcBroadcastPs0Inline).
    uint64_t bits = 0;
    std::memcpy(&bits, &value, sizeof(bits));
    return PpcBitCastToFloatInline(static_cast<uint32_t>(bits >> 32));
#elif defined(__x86_64__) || defined(_M_X64)
...
#if defined(_MSC_VER)
    // MSVC /O1//O2 mis-folds the lane extract here to zero (see PpcBroadcastPs0Inline).
    uint64_t bits = 0;
    std::memcpy(&bits, &value, sizeof(bits));
    return PpcBitCastToFloatInline(static_cast<uint32_t>(bits));
#elif defined(__x86_64__) || defined(_M_X64)
...
{
#if defined(_MSC_VER)
    // Integer bit-surgery: LOW 32 = ps1, HIGH 32 = ps0, matching the union layout bit for bit.
    // Avoids the _mm_unpacklo_ps/_mm_set_ss formulation MSVC /O2 mis-folds (see
    // PPC_PsMerge00Inline).
    const uint64_t packed = (static_cast<uint64_t>(PpcBitCastToU32Inline(ps0)) << 32) |
                            static_cast<uint64_t>(PpcBitCastToU32Inline(ps1));
    return PpcBitCastToDoubleInline(packed);
#elif defined(__x86_64__) || defined(_M_X64)
...
inline double PPC_PsMerge00Inline(double aValue, double bValue)
{
    ...
    // NOTE: implemented as integer bit-surgery on all targets. MSVC /O2 (UWP) mis-folds
    // every SIMD formulation of this helper (two-shuffle chain AND Get/Pack composition)
    // to literal zeros, zeroing all guest ps_merge00 results and blanking heap lyt panes.
    // LOW 32 = ps1, HIGH 32 = ps0.
    uint32_t aw[2];
    uint32_t bw[2];
    std::memcpy(aw, &aValue, 8);
    std::memcpy(bw, &bValue, 8);
    const uint32_t out[2] = { bw[1], aw[1] };
    double result;
...
inline double PPC_PsMerge01Inline(double aValue, double bValue)
{
    ...
    // Integer bit-surgery on all targets (see PPC_PsMerge00Inline note).
    uint32_t aw[2];
    uint32_t bw[2];
    std::memcpy(aw, &aValue, 8);
    std::memcpy(bw, &bValue, 8);
    const uint32_t out[2] = { bw[0], aw[1] };
    double result;
    std::memcpy(&result, out, 8);
    return result;
}
...
inline double PPC_PsMerge10Inline(double aValue, double bValue)
{
    ...
    std::memcpy(aw, &aValue, 8);
    std::memcpy(bw, &bValue, 8);
    const uint32_t out[2] = { bw[1], aw[0] };
    double result;
    std::memcpy(&result, out, 8);
    return result;
}
...
inline double PPC_PsMerge11Inline(double aValue, double bValue)
{
    ...
    std::memcpy(aw, &aValue, 8);
    std::memcpy(bw, &bValue, 8);
    const uint32_t out[2] = { bw[0], aw[0] };
    double result;
    std::memcpy(&result, out, 8);
    return result;
}
