...
std::filesystem::path ApplicationDataDirectory(std::string_view applicationName) {
    ...
    // Packaged (UWP/Xbox) apps: the OS rewrites the AppData environment
    // variables to the per-package LocalAppData location (the same
    // redirection the shell performs for packaged processes), so this
    // resolves to e.g. %LocalAppData%\<package-family>\<version>\Local.
    // If the variable is absent we fall through to current_path()
    // below; acceptable for the fallback case only.
    wchar_t localAppData[MAX_PATH] = {};
    if (GetEnvironmentVariableW(L"LOCALAPPDATA", localAppData, MAX_PATH) > 0) {
        return std::filesystem::path(localAppData) / applicationName;
    }
#elif defined(__APPLE__)
