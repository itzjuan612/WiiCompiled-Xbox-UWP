...
// Defined in `runtime/src/host_cpu_baseline.cpp`: the x86-64-v3 (AVX2) host-ISA guard. On the
// Clang/native path a priority-101 constructor runs it before main; MSVC has no GNU constructor
// attribute and the .CRT$XCT pre-main technique is dropped by the MSVC 14.44 linker, so this
// entry point calls it explicitly. It is idempotent and a no-op on non-x86-64.
...
// The SEM_* SetErrorMode mode constants are #defined only in the DESKTOP
// partition (winbase.h), but SetErrorMode itself is App-partition and the values
// are the stable Win32 constants below. Define them here only where the SDK
// does not already (i.e. the Store/UWP partition); native desktop gets them
// from winbase.h, so this is a no-op there.
...
// timeBeginPeriod/timeEndPeriod (mmsystem.h/timeapi.h) are gated to
// WINAPI_PARTITION_DESKTOP in the SDK; on the UWP Store partition they do not
// exist, and the timer API is the wrong tool on Xbox anyway (the timer
// resolution is fixed and the guest timing runs off QueryPerformanceCounter).
// On desktop builds this is a no-op change: the partition macro evaluates to 1.
...
// UCRT declares _pipe only in the desktop partition (corecrt_io.h gates it
// behind _CRT_USE_WINAPI_FAMILY_DESKTOP_APP). The CRT export is importable in
// a packaged app and _pipe creates an in-memory pipe (no pipe filesystem), so
// it is valid on UWP; prototype it here only where UCRT does not declare it.
#if defined(_WIN32) && !defined(_CRT_USE_WINAPI_FAMILY_DESKTOP_APP)
extern "C" int __cdecl _pipe(int* _PtHandles, unsigned int _PipeSize, int _TextMode);
#endif

bool InstallTranscriptPipe(int& outReadFd, int& outWriteFd, int targetFd) {
    // A stream can still be unbound here (see the Xbox note in InitializeProcessTranscript).
    // _dup2 raises the CRT invalid-parameter handler when given a negative descriptor, so
    // refuse instead of aborting the whole process.
    if (targetFd < 0) {
        return false;
    }
#if defined(_WIN32)
...
#else
    // A packaged (Store/UWP) app has no parent console to attach to: GetConsoleWindow and
    // AttachConsole are desktop-tier and absent from the App partition. The AppX process host
    // binds stdout/stderr when it wants them; otherwise NUL keeps the _fileno() valid so
    // InitializeProcessTranscript can redirect into the log file.
    const char* const sink = "NUL";
#endif
    if (!haveOut) {
...
void InitializeProcessTranscript(int argc, char** argv) {
    ...
    // A packaged console app (notably on Xbox) can reach this point with stdout and
    // stderr unbound: _fileno() is -1 and even freopen("NUL") fails, so _dup(-1)
    // below would trip the CRT invalid-parameter handler and abort the process
    // before anything else is logged. Bind such streams straight to the transcript
    // file first so their writes are captured and a valid descriptor exists.
#if defined(_WIN32)
    const auto bindUnboundStream = [&path](FILE* stream, int& savedFd) {
        const int fd = GetFileDescriptor(stream);
        if (fd >= 0) {
            savedFd = DuplicateFileDescriptor(fd);
            return;
        }
        if (std::freopen(RuntimeConfigFile::PathToUtf8(path).c_str(), "a", stream) != nullptr) {
            std::clearerr(stream);
        }
        savedFd = -1;
    };
    bindUnboundStream(stdout, state.savedStdoutFd);
    bindUnboundStream(stderr, state.savedStderrFd);
#else
...
#else
        // Non-Windows builds, and the Windows App (Store/UWP) partition where MessageBoxA is
        // desktop-tier and absent: keep them safe and retain the log diagnostic instead of a
        // native dialog.
...
#if !WINAPI_FAMILY_PARTITION(WINAPI_PARTITION_DESKTOP)
// The App partition's api-set import surface does not declare AddVectoredExceptionHandler,
// so a direct call would fail the UWP link. The handler *types* (PVECTORED_EXCEPTION_HANDLER,
// EXCEPTION_POINTERS) are in winnt.h for every partition, so resolve the function from
// kernelbase.dll at runtime and call through the pointer - no import-table entry is emitted.
// It is present on Xbox; a NULL on an unsupported dev box degrades to the top-level filter
// below (the flat guest-memory interception then cannot run, which is acceptable off-target).
typedef PVOID (WINAPI *MkwAddVehSehFn)(ULONG, PVECTORED_EXCEPTION_HANDLER);
static MkwAddVehSehFn ResolveSehAddVectoredExceptionHandler() {
    static MkwAddVehSehFn fn = nullptr;
    ...
    if (!probed) {
        ...
        if (kernelBase != nullptr) {
            fn = reinterpret_cast<MkwAddVehSehFn>(
...
#else
        const MkwAddVehSehFn addVeh = ResolveSehAddVectoredExceptionHandler();
        if (addVeh == nullptr) {
            RT_LOG(RT_TAG_RUNTIME) << "WARNING: AddVectoredExceptionHandler is unavailable on "
                                     "this UWP/Xbox system; flat guest-memory fault interception "
                                     "is disabled." << std::endl;
        } else {
            g_vectoredSehHandle = addVeh(1, SehLogger);
        }
#endif
...
int RuntimeMain(int argc, char** argv) {
#if defined(_MSC_VER)
    // Host-ISA guard (x86-64-v3/AVX2): MSVC has no GNU constructor attribute and the .CRT$XCT
    // pre-main technique is dropped by the MSVC 14.44 linker, so this entry point runs the check
    // - before the transcript, the SEH installers and, decisively, before any guest/AVX2
    // translated code executes. (Gated to MSVC so the native Clang object is unchanged: there the
    // priority-101 constructor runs it.)
    ...
    auroraConfig.prewarmMinFreeMb = RuntimeConfigFile::PrewarmMinFreeMb(0);
...
// only vulkan for linux
        ...
        // UWP has no DirectInput/RawInput/XInput joystick backends: Windows.Gaming.Input
        // is the only provider, and its driver in SDL is opt-in via this hint (default off).
