...
#else
    // The plain VirtualProtect is a FORCEINLINE shim over VirtualProtectFromApp in the App
    // partition, and that symbol is not in the UWP api-set import surface (it would LNK2019).
    // The importable VirtualProtectEx is the same effect on our own process, so use it here.
    return VirtualProtectEx(GetCurrentProcess(), address, static_cast<SIZE_T>(size),
                            protection, &previous) != FALSE;
#endif
#else
...
#if WINAPI_FAMILY_PARTITION(WINAPI_PARTITION_DESKTOP)
    const char* alloc2Name = "VirtualAlloc2";
    const char* map3Name = "MapViewOfFile3";
    const char* apiDescription = "VirtualAlloc2/MapViewOfFile3";
#else
    // The App (Store) partition does not export the plain VirtualAlloc2/MapViewOfFile3, but it
    // does export the *FromApp variants with the identical 7- and 9-argument shapes, so the same
    // function pointers (VirtualAlloc2Fn/MapViewOfFile3Fn) resolve them unchanged.
    const char* alloc2Name = "VirtualAlloc2FromApp";
    const char* map3Name = "MapViewOfFile3FromApp";
    const char* apiDescription = "VirtualAlloc2FromApp/MapViewOfFile3FromApp";
#endif
    // GetProcAddress takes an LPCSTR (it has no UNICODE->W mapping, unlike
    // GetModuleHandle/GetModuleFileName); the exported symbol names are ASCII,
    // so narrow names are correct on both the MSVC and the Clang/llvm-mingw
    // toolchains. (The GetModuleHandleW/LoadLibraryW above keep their wide
    // forms - those do have A/W variants.)
    HMODULE kernelBase = GetModuleHandleW(L"kernelbase.dll");
    if (kernelBase == nullptr) kernelBase = LoadLibraryW(L"kernelbase.dll");
    if (kernelBase != nullptr) {
        g_virtualAlloc2 =
            reinterpret_cast<VirtualAlloc2Fn>(GetProcAddress(kernelBase, alloc2Name));
        g_mapViewOfFile3 =
            reinterpret_cast<MapViewOfFile3Fn>(GetProcAddress(kernelBase, map3Name));
    }
    if (g_virtualAlloc2 == nullptr || g_mapViewOfFile3 == nullptr) {
        std::ostringstream oss;
        oss << "Flat guest memory requires Windows 10 1803 or newer (" << apiDescription
...
void EnsureReservation() {
    ...
    // Both partitions: a placeholder reservation that the per-region SplitPlaceholder /
    // MEM_REPLACE_PLACEHOLDER maps below carve into. The App-partition *FromApp entry points
    // do accept the placeholder flags (the earlier "plain MEM_RESERVE" UWP variant failed with
    // ERROR_INVALID_PARAMETER at the first MapViewOfFile3FromApp - a committed/reserved view
    // cannot overlap a plain non-placeholder reservation).
    const ULONG reserveFlags = MEM_RESERVE | kMemReservePlaceholder;
    // One extra granule stays uncommitted (unmapped on UWP) so an access that straddles
    // 0xFFFFFFFF faults instead of corrupting whatever the allocator happened to place
    // directly after the reservation.
    void* reserved = g_virtualAlloc2(GetCurrentProcess(), requested,
                                     static_cast<SIZE_T>(kGuestSpaceSize + kAllocationGranularity),
                                     reserveFlags, PAGE_NOACCESS,
                                     nullptr, 0);
    if (reserved == nullptr) {
        std::ostringstream oss;
...
//
// App partition (UWP): same placeholder dance as desktop, but through
// VirtualFreeEx - it is in the UWP api-set import surface (like
// VirtualProtectEx), while the plain VirtualFree placeholder overload is not.
...
#else
    VirtualFreeEx(GetCurrentProcess(), address, static_cast<SIZE_T>(size),
                  MEM_RELEASE | kMemPreservePlaceholder);
#endif
}
#endif
...
void MapGuestView(const Section& section, uint64_t sectionOffset, uint32_t guestBase,
    ...
    // Both partitions: replace the placeholder slice carved out above with the
    // section view (desktop MapViewOfFile3 / App MapViewOfFile3FromApp take the
    // identical flag set; the earlier App attempt without placeholders - either
    // MEM_RESERVE or a plain committed view - failed with ERROR_INVALID_PARAMETER
    // because a view cannot overlap a non-placeholder reservation).
    const ULONG mapAllocType = kMemReplacePlaceholder;
    void* view = g_mapViewOfFile3(section.handle, GetCurrentProcess(), target, sectionOffset,
                                  static_cast<SIZE_T>(mappedSize), mapAllocType,
...
bool CommitPlaceholder(uint8_t* address, uint64_t size, ProtectionFlags protection) {
    ...
    // Both partitions: commit privately into the placeholder slice carved out
    // above (App branch = VirtualAlloc2FromApp, resolved by ResolvePlacementApi;
    // identical flag set).
    const ULONG commitFlags = MEM_RESERVE | MEM_COMMIT | kMemReplacePlaceholder;
    void* result = g_virtualAlloc2(GetCurrentProcess(), address, static_cast<SIZE_T>(size),
                                   commitFlags, protection,
                                   nullptr, 0);
...
// AddVectoredExceptionHandler is not declared in the App api-set import surface, so calling it
// directly would fail the UWP link. The VEH handler *types* (PVECTORED_EXCEPTION_HANDLER,
// EXCEPTION_POINTERS) live in winnt.h and are available in every partition, so under UWP we
// resolve the function pointer from kernelbase.dll at runtime (it is present on Xbox; a NULL is
// an acceptable loud failure on an unsupported dev box) and call through it - no import-table
// entry is emitted, which is exactly what the Store link would reject.
#if !WINAPI_FAMILY_PARTITION(WINAPI_PARTITION_DESKTOP)
typedef PVOID (WINAPI *MkwAddVehFn)(ULONG, PVECTORED_EXCEPTION_HANDLER);
static MkwAddVehFn ResolveAddVectoredExceptionHandler() {
    static MkwAddVehFn fn = nullptr;
    ...
    if (!probed) {
        ...
        if (kernelBase == nullptr) kernelBase = LoadLibraryW(L"kernelbase.dll");
        if (kernelBase != nullptr) {
            fn = reinterpret_cast<MkwAddVehFn>(
...
#else
    MkwAddVehFn addVeh = ResolveAddVectoredExceptionHandler();
    if (addVeh == nullptr) {
        throw std::runtime_error(
            "AddVectoredExceptionHandler is unavailable on this UWP/Xbox system, so the flat "
            "guest-memory fault interception cannot run.");
    }
    g_vectoredHandle = addVeh(1, FlatGuestVectoredHandler);
#endif
