...
#endif
#include <windows.h>
// The App (Store/UWP) partition gates the CreateFileA *declaration* behind
// DESKTOP|SYSTEM|GAMES in fileapi.h (the Store variant is CreateFile2), but the
// symbol is importable (proven by Launcher/uwp-smoke/pipe_createfile_probe.bat).
// This file uses CreateFileA to open the Discord IPC named pipes, which do not
// exist in a packaged app, so it fails cleanly (INVALID_HANDLE_VALUE) and
// presence is unavailable - the correct UWP/Xbox behavior. Native desktop gets
// CreateFileA from fileapi.h, so this is a no-op there.
#if !WINAPI_FAMILY_PARTITION(WINAPI_PARTITION_DESKTOP | WINAPI_PARTITION_SYSTEM | WINAPI_PARTITION_GAMES)
extern "C" HANDLE WINAPI CreateFileA(LPCSTR lpFileName, DWORD dwDesiredAccess, DWORD dwShareMode,
