...
#endif
#include <windows.h>
// The App (Store/UWP) partition gates the CreateFiber/ConvertThreadToFiber
// *declarations* behind WINAPI_PARTITION_DESKTOP in winbase.h, but the symbols
// are importable (proven by Launcher/uwp-smoke/fiber_import_probe.bat, which
// links). The other fiber APIs (SwitchToFiber/DeleteFiber/ConvertFiberToThread)
// are already App-partition and GetCurrentFiber is a TEB inline (winnt.h), so
// only these two need a local prototype on UWP. Native desktop (and the
// SYSTEM/GAMES partitions) get them from the SDK, so this is a no-op there.
