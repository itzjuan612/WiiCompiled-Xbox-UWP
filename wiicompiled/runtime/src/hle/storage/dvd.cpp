...
static void ScanDirectory(const fs::path& root, const std::string& virtualPrefix,
    ...
    const auto registerEntry = [&](const fs::directory_entry& entry) {
        ...
        // Purely lexical: the entry was enumerated from `root`, so no
        // canonicalization is needed. fs::relative() re-walks the path with
        // filesystem I/O, which an AppContainer (UWP/Xbox package) denies
        // with ERROR_ACCESS_DENIED for every file, silently emptying the
        // whole disc index.
