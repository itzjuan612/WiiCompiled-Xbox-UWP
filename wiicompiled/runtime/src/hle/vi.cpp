...
void VI_HLE_PresentFrame(bool presentedXfb, bool paceToRetrace) {
    ...
    if ((++s_diagFrames % 300u) == 0u) {
        ...
        AuroraPresentTiming pt{};
        aurora_get_present_timing(&pt);
        const char* driver = SDL_GetCurrentVideoDriver();
        int winCount = 0;
        SDL_Window** wins = SDL_GetWindows(&winCount);
        RT_LOG(RT_TAG_VI) << "diag: presented=" << st->presentedFrameCount
                          << " fps=" << pt.framesPerSecond
        ...
        for (int i = 0; i < winCount; ++i) {
            ...
            RT_LOG(RT_TAG_VI) << " | win#" << i << " id=" << SDL_GetWindowID(wins[i])
                              << " size=" << w << "x" << h
                              << " flags=0x" << std::hex << SDL_GetWindowFlags(wins[i]) << std::dec;
        }
        SDL_free(wins);
        int joyCount = 0;
        SDL_JoystickID* joys = SDL_GetJoysticks(&joyCount);
        SDL_free(joys);
        int padCount = 0;
        SDL_JoystickID* pads = SDL_GetGamepads(&padCount);
        SDL_free(pads);
        RT_LOG(RT_TAG_VI) << " | joysticks=" << joyCount << " gamepads=" << padCount << std::endl;
    }
