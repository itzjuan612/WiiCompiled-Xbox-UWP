...
// The SChannel credential/context/stream-size state is only present on the
// desktop Windows partition. The Store/App (UWP/Xbox) partition gates all
// SChannel types (CredHandle, SecPkgContext_StreamSizes, ...) behind
// WINAPI_PARTITION_DESKTOP in the SDK, so they are absent there; on that
// partition the session degrades to the stub SSL implementation below
// (handshake/read/write report SSL_ERR_FAILED), exactly as on non-Windows.
// On native desktop Windows, WINAPI_PARTITION_DESKTOP is 1 (and when
// WINAPI_FAMILY is undefined the SDK defaults it to 1), so these fields are
// present and the SChannel path is byte-identical to before.
...
// SChannel is the host TLS backend on desktop Windows only. On the Store/App
// (UWP/Xbox) partition every SChannel function and type is gated behind
// WINAPI_PARTITION_DESKTOP in the SDK and unavailable, so this whole backend is
// excluded there and the stub Ssl* implementations (below) are used instead.
// On native desktop Windows WINAPI_PARTITION_DESKTOP is 1, so this is the
// original #ifdef _WIN32 path unchanged.
#if defined(_WIN32) && WINAPI_FAMILY_PARTITION(WINAPI_PARTITION_DESKTOP)
