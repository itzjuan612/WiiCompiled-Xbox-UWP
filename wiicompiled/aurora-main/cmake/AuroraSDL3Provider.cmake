...
elseif (_aurora_sdl3_provider STREQUAL "vendor")
  ...
  if (NOT TARGET SDL3-static AND NOT TARGET SDL3-shared)
    ...
    if (WIN32)
      ...
      if (AURORA_SDL3_LIBUSB AND NOT MKW_PLATFORM_UWP)
        include("${CMAKE_CURRENT_LIST_DIR}/AuroraLibUSB.cmake")
      endif ()
    endif ()

    # UWP (Option B) uses the SternXD/SDL3-uwp fork, not vanilla SDL: the fork's
    # C++/CX WinRT driver supplies gamepad/pointer/keyboard via the Xbox Game
    # Bar, so HIDAPI (and the Wii-HIDAPI source patch) are unnecessary and would
    # also fail — the patch's anchors target vanilla 3.4.4, not the fork.
    if (MKW_PLATFORM_UWP)
      set(SDL_HIDAPI OFF CACHE BOOL "WinRT driver handles controllers; no HIDAPI on UWP" FORCE)
      set(AURORA_SDL3_LIBUSB OFF CACHE BOOL "" FORCE)
    endif ()
