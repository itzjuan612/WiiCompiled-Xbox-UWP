...
// RtlGetVersion is resolved dynamically from ntdll instead of linked
// directly. Under the UWP (Option B) MSVC target the um/x64 import libs
// expose no RtlGetVersion stub (it is an ntdll/Winternl-level API, not part
// of the UWP API contract), so a direct __imp_RtlGetVersion reference left
// the build14 link with one LNK2019. Runtime resolution through
// GetProcAddress is what SDL's own UWP path does (SDL_windows.c 315-327),
// and ntdll.dll is loadable in the UWP app container. The previous
// `extern "C" NTSYSAPI NTSTATUS NTAPI RtlGetVersion(...)` declaration is
// gone entirely; both toolchains now resolve the function at runtime.
#elif __APPLE__
...
std::string GetOSVersion() {
  ...
  if (!ntdll) {
    Log.error("Unable to load ntdll.dll");
    return Unknown;
  }
  const auto rtlGetVersion = reinterpret_cast<NTSTATUS (NTAPI *)(
    PRTL_OSVERSIONINFOEXW)>(GetProcAddress(ntdll, "RtlGetVersion"));
  if (!rtlGetVersion) {
    Log.error("RtlGetVersion is unavailable in ntdll.dll");
    return Unknown;
  }
  const auto result = rtlGetVersion(&info);
  if (result != 0) {
