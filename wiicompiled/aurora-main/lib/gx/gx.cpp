...
// ponytail: A fixed direct-mapped cache avoids allocator/hash-table overhead. Sized down on
// UWP/Xbox, where every cached entry pins a GPU texture and the console runs near its memory ceiling.
#if defined(_WIN32) && defined(WINAPI_FAMILY) && (WINAPI_FAMILY == WINAPI_FAMILY_APP)
constexpr size_t kTextureResolveIdentityCacheSize = 512;
#else
constexpr size_t kTextureResolveIdentityCacheSize = 2048;
#endif
static std::array<TextureResolveIdentityCacheEntry, kTextureResolveIdentityCacheSize>
    s_textureResolveIdentityCache{};
...
#if defined(_WIN32) && defined(WINAPI_FAMILY) && (WINAPI_FAMILY == WINAPI_FAMILY_APP)
constexpr size_t kStaticTextureSourceCacheHardLimit = 256;
#else
constexpr size_t kStaticTextureSourceCacheHardLimit = 512;
#endif
...
gfx::TextureHandle resolve_static_texture(const GXTexObj_& obj) {
  ...
  if (canUseSourceCache) {
    ...
    if (auto it = s_staticTextureSourceCache.find(sourceKey); it != s_staticTextureSourceCache.end()) {
      if (!validate_static_texture_source_entry(sourceKey, it->second)) {
        // The source bytes changed while the key stayed the same (a THP movie frame decoded
        // into a reused guest buffer, or an in-place texture update). Re-upload into the
        // existing texture instead of minting a new one, which would otherwise allocate a
        // fresh GPU texture every frame and exhaust D3D12 memory.
        const auto handle = it->second.handle;
        if (handle && !handle->isReplacement) {
          const u32 dataSize = static_texture_data_size(sourceKey);
          gfx::write_texture(*handle, {static_cast<const uint8_t*>(sourceKey.data), dataSize});
          it->second = make_static_texture_source_entry(sourceKey, handle);
          if (obj.texObjId != 0) {
            record_tex_obj_source_key(obj.texObjId, sourceKey);
          }
  ...
  if (canCacheUpload) {
    ...
    if (canUseSourceCache) {
      if (s_staticTextureSourceCache.size() >= kStaticTextureSourceCacheHardLimit) {
...
//
// On UWP/Xbox the console shares its physical memory between the CPU and GPU and runs close to the
// ceiling: a race mints new texture objects faster than the idle sweeper retires them, the caches
// grow toward their limits (a 304x232 RGBA8 texture is ~280 KiB) and a first-use pipeline compile
// then fails with FXC E_OUTOFMEMORY. Keep the caches much smaller there so cached GPU memory stays
// bounded; entries are re-minted on demand, which is cheaper than running out of memory.
#if defined(_WIN32) && defined(WINAPI_FAMILY) && (WINAPI_FAMILY == WINAPI_FAMILY_APP)
constexpr u64 kTextureCacheIdleRevisions = 64;
constexpr size_t kTextureObjectCacheSoftLimit = 256;
constexpr size_t kTlutObjectCacheSoftLimit = 128;
constexpr size_t kTexObjSourceKeySoftLimit = 512;
#else
...
#if defined(_WIN32) && defined(WINAPI_FAMILY) && (WINAPI_FAMILY == WINAPI_FAMILY_APP)
  // The resolve-identity and source-front caches are revision-gated: an entry is only reusable
  // while its stamp matches the current revision, so after a bump every old entry is dead but its
  // stored TextureBind still pins the GPU texture. On the console that pinned memory is the bulk
  // of the budget, so drop the stale strong references here (the entries are rebuilt on demand).
  for (auto& entry : s_textureResolveIdentityCache) {
    if (entry.valid && entry.validationRevision != revision) {
      entry.valid = false;
      entry.binding.reset();
    }
  }
