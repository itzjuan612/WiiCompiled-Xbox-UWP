...
// WINAPI_FAMILY_PARTITION (a Windows SDK / mingw-w64 macro) is used below to
// gate desktop-only code. Include its header explicitly and only where it
// exists: the UWP/MSVC build gets it from the SDK shared\ dir and the native
// llvm-mingw build from the mingw-w64 runtime (there it is the identity macro
// WINAPI_FAMILY_PARTITION(x) x with WINAPI_PARTITION_DESKTOP true on desktop,
// i.e. the historical native semantics). Without it the macro is undefined on
// this TU's include path (the windows.h include that used to provide it moved
// below the first use in commit 1a354e7).
#if defined(_WIN32)
...
// The native window-proc hook (HWND/WNDPROC/SetWindowLongPtr/DefWindowProc/CallWindowProc) is
// desktop-tier and absent from the Windows App (Store/UWP) partition, so the whole block below is
// desktop-only. Under UWP resize detection falls back to the SDL_EVENT_WINDOW_RESIZED path already
// wired in lifecycle_event_watch (g_nativeResizePending), and query_native_client_size returns
// false so callers use get_window_size().
#if defined(_WIN32) && WINAPI_FAMILY_PARTITION(WINAPI_PARTITION_DESKTOP)
